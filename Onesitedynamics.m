...
tau = 0.11;                   % in cm^-1
del_tilde = 1.5;             % dimentionless quantity
...
Xi_on_values=-10*tau:0.05:10*tau;
CM_shift_values=zeros(1,length(Xi_on_values));
Xi_off=0;

for jj=1:1:length(Xi_on_values)
    Xi_on=Xi_on_values(jj);
    Ei=E_old;
...
% H_dis=Xi_on*H_dis;
site_number=N/2-1;
H_dis=one_site_disorder(N,site_number,Xi_on);
...
% title('Evolution of the initial state for \Xi_{on} = ',[num2str(Xi_on)]);
% 
...
% ylabel('X_{cm}');
% xlabel('z'); 
% title('Centre of mass for \Xi_{on} = ',[num2str(Xi_on)]);
% grid off;



CM_shift_values(1,jj)=abs(CM(end)-CM(1));

end

figure(10);
plot(Xi_on_values/tau,CM_shift_values,'o');
xlabel('\Xi_{on}/\tau');
