...
Xi_on_values = 0:0.01:0.5; % Xi_on values to iterate over

%% Number of trials for averaging
num_trials = 10;

%% Initialize storage for CM_shift over trials
CM_shift_all = zeros(length(g_values), length(Xi_on_values), num_trials);
CM_shift_linear_all = zeros(length(Xi_on_values), num_trials);
...
for trial = 1:num_trials
    ...
    CM_shift_linear = zeros(1, length(Xi_on_values));

    parfor kk = 1:length(Xi_on_values)
        Xi_on = Xi_on_values(kk);
        H_dis = Xi_on * H_dis_base;
...
% 
% for jj = 1:length(g_values)
%     plot(Xi_on_values/tau, CM_shift_mean(jj, :)/N_periods, 'o-', 'Color', colors(jj, :), 'LineWidth', 2, ...
%          'DisplayName', ['g = ', num2str(g_values(jj))]);
% end
% 
% plot(Xi_on_values/tau, CM_shift_linear_mean/N_periods, 'o', 'DisplayName', 'Linear', 'Color', 'k');
% 
% xlabel('Xi_{on}/ \tau');
% ylabel('CM_{shift}');
...
for jj = 1:length(g_values)
    ...
    errorbar(Xi_on_values/tau, CM_shift_mean(jj, :)/N_periods, CM_shift_std(jj, :)/N_periods, ...
        'o', 'Color', colors(jj, :), 'LineWidth', 1.5, 'MarkerFaceColor', colors(jj, :), ...
        'DisplayName', ['g = ', num2str(g_values(jj))]);
end

% Error bar for linear case
errorbar(Xi_on_values/tau, CM_shift_linear_mean/N_periods, CM_shift_linear_std/N_periods, ...
